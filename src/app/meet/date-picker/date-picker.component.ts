...
export class DatePickerComponent implements OnInit, OnDestroy {
  ...
  monthSubscription: Subscription | undefined = this.datePickerForm
    .get('month')
    ?.valueChanges.subscribe((month) =>
      this.datePickerService.updateMonth(month)
    );
  yearSubscription: Subscription | undefined = this.datePickerForm
    .get('year')
    ?.valueChanges.subscribe((year) => this.datePickerService.updateYear(year));
