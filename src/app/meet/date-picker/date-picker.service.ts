...
import { DatePickerDay } from '../models/common';
...
export class DatePickerService {
  ...
  private calendarDaysSubject_: Subject<DatePickerDay[]> = new BehaviorSubject<
    DatePickerDay[]
  >([]);
  public calendarDays$: Observable<DatePickerDay[]> =
    this.calendarDaysSubject_.asObservable();

  // the latest day the user has selected--not month or year
  private selectedDateSubject_: Subject<number> = new BehaviorSubject<number>(
    DateTime.now().day
  );
  public selectedDate$: Observable<number> =
    this.selectedDateSubject_.asObservable();
  ...
  private selectedMonthSubject_: Subject<string> = new BehaviorSubject<string>(
    DateTime.now().toFormat('MMM')
  );
  public selectedMonth$: Observable<string> =
    this.selectedMonthSubject_.asObservable();
  ...
  private selectedYearSubject_: Subject<number> = new BehaviorSubject<number>(
    DateTime.now().year
  );
  public selectedYear$: Observable<number> =
    this.selectedYearSubject_.asObservable();

  // displayed dates
  private displayedYearSubject_: Subject<number> = new BehaviorSubject<number>(
    DateTime.now().year
  );
  public displayedYear$: Observable<number> =
    this.displayedYearSubject_.asObservable();

  private displayedMonthSubject_: Subject<string> = new BehaviorSubject<string>(
    DateTime.now().toFormat('MMM')
  );
  public displayedMonth$: Observable<string> =
    this.displayedMonthSubject_.asObservable();

  private displayedDaySubject_: Subject<number> = new BehaviorSubject<number>(
    DateTime.now().day
  );
  public displayedDay$: Observable<number> =
    this.displayedDaySubject_.asObservable();
  ...
  updateMonthYearhSub = combineLatest([this.selectedYear$, this.selectedMonth$])
    .pipe(
      map(([year, month]) => {
        const dt = DateTime.fromFormat(`${year}-${month}`, 'yyyy-MMM').startOf(
          'day'
        );

        // Monday is 1, so Sunday must be 0
        const numPaddedDays = dt.weekday % 7;
        const paddedDays: string[] = Array.from(
          Array(numPaddedDays).keys()
        ).map(() => '');
        const actualDays: string[] = Array.from(
          Array(dt.daysInMonth).keys()
        ).map((n) => `${n + 1}`);
        const allDays = paddedDays.concat(actualDays);

        // TODO: eventually refactor this to use this.computeCalendarDays()
        const allDatePickerDays: DatePickerDay[] = allDays.map<DatePickerDay>(
          (displayValue) => {
            const todaysDate = DateTime.now().startOf('day').toMillis();
            const iteratedDate = DateTime.fromFormat(
              `${year}-${month}-${displayValue}`,
              'yyyy-MMM-d'
            )
              .startOf('day')
              .toMillis();
  ...
  mnotyYearSub = this.selectedDate$
    .pipe(withLatestFrom(this.monthYear))
    .subscribe((dates) => {
      const day = dates[0];
      const dayNumber: string = `${DateTime.fromFormat(`${day}`, 'd').toFormat(
        'dd'
      )}`;
      const month = dates[1][0];
      const monthNumber: string = `${DateTime.fromFormat(
        `${month}`,
        'MMM'
      ).toFormat('MM')}`;
      ...
      const calendarDays: DatePickerDay[] = this.computeCalendarDays(
        year,
        month,
        day
      );
  ...
  private computeCalendarDays(
    ...
    const paddedDays: string[] = Array.from(Array(numPaddedDays).keys()).map(
      () => ''
    ); // blank days on the calendar before actual days
    const actualDays: string[] = Array.from(Array(dt.daysInMonth).keys()).map(
      (n) => `${n + 1}`
    );
    const allDays = paddedDays.concat(actualDays);
